...
if(isset($_POST['login'])){
    ...
    $result = $conn->prepare("SELECT user_id, first_name, last_name, user_email, user_password, is_verified FROM users WHERE user_email = ? LIMIT 1");
    $result->bind_param('s', $email);
    
    if($result->execute()){
        $result->bind_result($user_id, $first_name, $last_name, $user_email, $user_password, $is_verified); 
        $result->store_result();
        ...
        if($result->num_rows() == 1){
            ...
            if(password_verify($password, $user_password)){
                if($is_verified == 1){ // Check if the account is verified
                    ...
                    header('location: acc.php?message=success');
                } else {
