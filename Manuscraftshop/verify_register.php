...
if (isset($_POST['verify'])) {
    ...
    if ($is_verified == 1) {
        header('location: acc.php?message=Email already verified');
        exit();
    }
    ...
    if ($input_code === $stored_code) {
        ...
        if ($stmt->execute()) {
            ...
            $stmt = $conn->prepare("SELECT user_id, first_name, last_name FROM users WHERE user_email = ?");
            $stmt->bind_param('s', $email);
            $stmt->execute();
            ...
            header('location: acc.php?msg=register');
            exit();
        } else {
            ...
            <div class="mx-auto container">
                <form id="register-form" action="verify_register.php" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" style="width: 50%;" maxlength="6" name="verification_code" required>
                    </div>
